...
def transform_clean_pop4(file_path, **kwargs):
    try:
        ...
        if 'NIVEAU' in df.columns and 'C_SEXE' in df.columns and 'C_AGEQ80_14' in df.columns and 'C_COUPLE' in df.columns:
            df = df[['NIVEAU', 'CODGEO', 'C_SEXE', 'C_AGEQ80_14', 'C_COUPLE', 'NB']]
            df.columns = ['nivgeo', 'codgeo', 'sexe', 'ageq80_14', 'couple', 'nb']
        elif 'NIVEAU' in df.columns and 'C_SEXE' in df.columns and 'C_AGEQ80' in df.columns and 'C_COUPLE' in df.columns:
            df = df[['NIVEAU', 'CODGEO', 'C_SEXE', 'C_AGEQ80', 'C_COUPLE', 'NB']]
            df.columns = ['nivgeo', 'codgeo', 'sexe', 'ageq80_14', 'couple', 'nb']
        elif 'NIVGEO' in df.columns and 'SEXE' in df.columns and 'AGEQ80_14' in df.columns and 'COUPLE' in df.columns:
            df = df[['NIVGEO', 'CODGEO', 'SEXE', 'AGEQ80_14', 'COUPLE', 'NB']]
            df.columns = ['nivgeo', 'codgeo', 'sexe', 'ageq80_14', 'couple', 'nb']
        else:
...
with DAG(
    ...
    description='Transform, clean, map, and integrate data for pop4',
    schedule_interval=None,
    start_date=days_ago(1),
    catchup=False
) as dag:
    prepare_table_task = PythonOperator(
        task_id='prepare_table_pop4',
        python_callable=prepare_table,
        op_args=['{{ dag_run.conf["millesime"] }}', table_name],
    )

    transform_clean_task = PythonOperator(
        task_id='transform_clean_pop4',
        python_callable=transform_clean_pop4,
    ...
    load_task = PythonOperator(
        ...
        op_args=[
            '{{ task_instance.xcom_pull(task_ids="transform_clean_pop4") }}', 
            table_name
        ],
    )
