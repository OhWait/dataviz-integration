...
def transform_clean_pop6(file_path, **kwargs):
    try:
        ...
        if 'NIVEAU' in df.columns and 'C_SEXE' in df.columns and 'C_AGEQ65' in df.columns and 'C_CS1_8' in df.columns:
            df = df[['NIVEAU', 'CODGEO', 'C_SEXE', 'C_AGEQ65', 'C_CS1_8', 'NB']]
            df.columns = ['nivgeo', 'codgeo', 'sexe', 'ageq65', 'cs1_8', 'nb']
        elif 'NIVGEO' in df.columns and 'CS1_8' in df.columns and 'AGEQ65' in df.columns and 'SEXE' in df.columns:
            df = df[['NIVGEO', 'CODGEO', 'CS1_8', 'AGEQ65', 'SEXE', 'NB']]
            df.columns = ['nivgeo', 'codgeo', 'cs1_8', 'ageq65', 'sexe', 'nb']
        else:
...
with DAG(
    ...
    description='Transform, clean, map, and integrate data for pop6',
    schedule_interval=None,
    start_date=days_ago(1),
    catchup=False
) as dag:
    prepare_table_task = PythonOperator(
        task_id='prepare_table_pop6',
        python_callable=prepare_table,
        op_args=['{{ dag_run.conf["millesime"] }}', table_name],
    )

    transform_clean_task = PythonOperator(
        task_id='transform_clean_pop6',
        python_callable=transform_clean_pop6,
    ...
    load_task = PythonOperator(
        ...
        op_args=[
            '{{ task_instance.xcom_pull(task_ids="transform_clean_pop6") }}', 
            table_name
        ],
    )
